...
function Band({
  ...
  const texture = useTexture(lanyardImage || LANYARD_PNG_URL) as THREE.Texture;
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: HTMLImageElement | HTMLCanvasElement, rect: { x: number; y: number; w: number; h: number }) => {
      const rx = rect.x * W;
      const ry = rect.y * H;
      const rw = rect.w * W;
      const rh = rect.h * H;
  ...
  // Bounce / swing impulse when scrolled into view
  useEffect(() => {
    if (triggerBounce && card.current) {
      [card, j1, j2, j3, fixed].forEach(ref => ref.current?.wakeUp());
      const timer = setTimeout(() => {
        if (card.current) {
          card.current.wakeUp();
          card.current.applyImpulse({ x: 3.2, y: 7.0, z: -1.2 }, true);
          card.current.applyTorqueImpulse({ x: 0.15, y: 0.6, z: 0.25 }, true);
        }
      }, 150);
  ...
  return (
    <>
      <group position={[0, 3.2, 0]}>
        ...
        <RigidBody position={[1.0, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[1.1, 1.55, 0.01]} />
          <group
            scale={3.2}
            position={[0, -1.7, -0.05]}
